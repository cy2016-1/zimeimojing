...
class plugin_list(ApiBase):
    ...
    def main(self):
        ...
        if self.query['op']=='getlist':
            self.send({'MsgType': 'Text', 'Receiver': 'Screen', 'Data': '铜陵的天气'})
            # self.send('Text', 'Screen','显示这个吧')
            #self.send('Text', 'Chat','显示这个吧')
        ...
        # 单个插件升级更新信息
        if self.query['op']=='getupinfo':
            name = self.query['name']       # 插件名称
            userid = self.query['userid']     # 用户ID
