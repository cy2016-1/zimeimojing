...
class admin_index(RequestInit):
    ...
    def network_detection(self):
        try:
            response = requests.post(
                self.config['httpapi'] + r'/raspberry/ping.html', timeout=10)
    ...
    def main(self):
        # 获取所有设备状态信息
        if self._GET['op'] == 'getallinfo':
            return self.get_allinfo()

        if self._GET['op'] == 'getRotate':
            ret_arr = {
                'code': 20000,
                'message': '获取屏幕方向信息成功',
                'data': self.getRotate()
        ...
        if self._GET['op'] == 'wifi_name_list':
            ret_arr = {
                'code': 20000,
        ...
        if self._GET['op'] == 'network_detection':
            ret_arr = {
                'code': 20000,
                'message': '获取网络状态成功',
