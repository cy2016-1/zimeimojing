...
class ControlCenter(MsgProcess):
    ...
    def LoadAll(self):
        ...
        for (module, isEnable) in (self.config["LoadModular"]).items():
            if isEnable:
                package = r'python.package.' + module
                try:
        ...
        if all(map(lambda p: p.name != pluginName, self.ProcessPool)):  # 如果插件没有运行
            ...
            with open(configfile) as fd:
                ...
                try:
                    ...
                    logging.error('插件[%s]模块加载失败! %s' % (pluginName, e))
                    return
...
if __name__ == "__main__":
    ...
    sys.path.append(os.path.join(cwd, 'python/api'))
    sys.path.append(os.path.join(cwd, 'python/bin'))
    sys.path.append(os.path.join(cwd, 'python/module'))
    sys.path.append(os.path.join(cwd, 'python/include'))
    sys.path.append(os.path.join(cwd, 'webroot/api'))
    del cwd
