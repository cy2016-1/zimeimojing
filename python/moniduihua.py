...
class Hecheng(Base):
    ...
    def main(self,reobj):
        ...
        try:
           ...
           # log.info('Web浏览器测试' + self.TTS_URL + '?' + data)
            ...
            try:
                f = urlopen(req,timeout=4)#合成时间不多，识别最耗时
                result_str = f.read()

                save_file = './result.' + self.FORMAT
    ...
    def my(self,where,txt):

        txt = {'obj':where,'msg': txt}
        data = mysocket.Mysocket()
        data.ws = create_connection("ws://localhost:8103")
        data.send_info(txt)
...
if __name__ == '__main__':
    ...
    if len(sys.argv)>1:
        ...
        print("缺少txt文件")
        exit()
    print("选中文件:",argv,"退出回复q")
    with open( argv, encoding='utf-8') as xx:
        while 1:
            y = xx.readline()
            if len(y) == 0:
                exit()
            if len(y)==1:
                ...
                pass_ = input("我说完回车--->/"+''.join(data))
                if pass_ == "q" or  pass_ == "Q":
                    exit()
                lei.my('zhuren',''.join(data))

            elif y[0] == "#":
                pass_ = input("我说完回车--->/"+y[1:])
                if pass_ == "q" or  pass_ == "Q":
                    exit()
                lei.my('zhuren',y[1:])
                ...
                pass_ = input("魔镜说话--->回车开始/"+y)
                if pass_ == "q" or  pass_ == "Q":
                    exit()
                lei.main(y)
                lei.my('mojing',y)
                os.system("sudo aplay ./result.wav")
