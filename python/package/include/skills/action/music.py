...
class Music():
    def __init__(self,public_obj):
        #唤醒内存变量
        self.is_snowboy = public_obj.is_snowboy
        #前端通讯变量
        self.sw         = public_obj.sw
        #停止歌曲变量
        ...
        if len(res) <=2:
            ...
            #唤醒后暂停音乐进程
            snowboy_stop_music_process = mp.Process(target = self.snowboy_stop_music )#创建进程1
            #pl.daemon = True #定义守护进程
            snowboy_stop_music_process.start()#启动进程1
        else:
            print("音乐存在")
            #for x in res:
            #    pid = x[0]
    ...
    def postmojing(self,txt):
        ...
        try:
            self.sw.send_info( {'obj':'mojing','msg': txt} )
        except:
            pass
    #当前音量
    def voices(self):
...
#==================================================
    ...
    def start_music(self):
        print("进入start_music")
        #修改进程名称
        setproctitle.setproctitle("mojingmusic")
        rfPath = "/mojingmusic"
        while 1:
            try:
                rp = open(rfPath, 'r')
                ...
                response[:4] =="播放歌曲" or response[:4] =="播放音乐" or response[2:] =="。":
                    if response[2:] == "。" or  response[4:] == "。" :
                        ...
                        self.start_play(response[4:] )
                elif response[:2] =="播放":
                    self.start_play(response[2:])
