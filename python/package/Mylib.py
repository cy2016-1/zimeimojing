...
class Mylib:
    ...
    # http_get访问
    @staticmethod
    def http_get(url, postdata={}, timeout=5):
        res = {'code': '9999', 'msg': '[error:Mylib.http_get]网络请求失败！', 'data': ''}
        try:
            response = requests.get(url, params=postdata, timeout=5)
    ...
    def http_post(url, postdata={}, timeout=5):
        ...
        try:
            response = requests.post(url, data=postdata, timeout=5)
            if int(response.status_code) == 200:
                res['code'] = '0000'
                res['msg']  = 'POST网络请求成功！'
                res['data'] = response.text
    ...
    def http_urllib(url, postdata={}, timeout=5):
        try:
            res = {'code': '9999', 'msg': '[error:Mylib.http_urllib]网络请求失败！', 'data': ''}
